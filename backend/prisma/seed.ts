...
async function main() {
  ...
  const harshUser = await upsertSeedUser({
    email: "harsh@hrms.local",
    name: "Harsh Kumar",
    password: seedEmployeePassword,
    role: employeeRole,
    createdRoles
  ...
  const harshEmployee = await prisma.employee.upsert({
    where: {
  ...
  await Promise.all(
    [employee, hrEmployee, managerEmployee, selfServiceEmployee, harshEmployee].flatMap(
      (seedEmployee) =>
  ...
  await prisma.announcement.upsert({
    ...
    update: {
      title: "Dashboard and reports are available",
  ...
  await prisma.notification.upsert({
    where: {
      id: "seed-harsh-self-service-ready"
    },
    update: {
      userId: harshUser.id,
      title: "Employee self-service ready",
  ...
  await prisma.feedback.upsert({
    where: {
      id: "seed-admin-feedback"
    },
    update: {
      employeeId: employee.id,
      authorId: employee.id,
      category: "PRAISE",
      message: "Performance records are ready for appraisal tracking.",
      isPrivate: false
    },
    create: {
      id: "seed-admin-feedback",
