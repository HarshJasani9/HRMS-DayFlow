...
const smokeEmployeeEmail = "smoke.employee@hrms.local";
...
async function runSmoke(baseUrl: string): Promise<void> {
  ...
  await check("employee profile, detail, and document upload", async () => {
    ...
    assert(
      Boolean(detail.employee.documents?.some((document) => document.fileName === "smoke-test-doc.txt")),
