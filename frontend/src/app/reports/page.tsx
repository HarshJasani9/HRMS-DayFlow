...
import { BarChart3, CalendarDays, Clock3, DollarSign, Users } from "lucide-react";
...
function ReportsContent({ user, token }: ReportsContentProps) {
  ...
  const payrollQuery = useQuery({
    queryKey: ["report-payroll", token, year],
    queryFn: () => getPayrollReportData(token, { year }),
    retry: false
  });
  ...
  const payrollReport = payrollQuery.data?.success ? payrollQuery.data.data : null;
    ...
    <AppShell user={user} token={token}>
      <div className="space-y-6">
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatusCard
            label="Payroll Cost"
            value={formatMoney(payrollReport?.summary.totalNet ?? 0)}
            detail={`${payrollReport?.summary.totalRuns ?? 0} runs in ${year}`}
            icon={DollarSign}
            tone="slate"
          />
        </section>
